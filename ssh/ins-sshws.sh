...
if [ -f "/etc/systemd/system/aNzVPNsshws.service" ]; then
clear
else
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/jalgitap/mpek/main/ssh/proxy3.js"
cat <<EOF> /etc/systemd/system/aNzVPNsshws.service
[Unit]
Description=WSenabler
Documentation=dotycatvpn@copyright
...
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/jalgitap/mpek/main/ssh/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler && /usr/bin/ssh-wsenabler
systemctl daemon-reload >/dev/null 2>&1
systemctl enable aNzVPNsshws.service >/dev/null 2>&1
systemctl start aNzVPNsshws.service >/dev/null 2>&1
...
PID=`ps -ef |grep -v grep | grep sshws |awk '{print $2}'`
if [[ ! -z "${PID}" ]]; then
systemctl stop aNzVPNsshws.service
